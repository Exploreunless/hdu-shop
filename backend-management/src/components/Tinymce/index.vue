<template>
  <editor id="tinymceId" v-model="content"  tag-name="div" :init="init" />
...
export default {
  ...
  setup(props,context) {
    ...
    const revert_data=(content)=>{
      ...
      context.emit('update:modelValue',unref(content))
    }

    const imageSuccessCBK = (arr)=> {
        arr.forEach(v => {
          tinymce.get('tinymceId').insertContent(`<img class="wscnph" src="${v.url}" >`)
        })
        revert_data(content);
    }
    const content = ref('');
